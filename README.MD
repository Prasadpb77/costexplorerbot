...
🏗️ Architecture
User → API Gateway → Lambda Function → AWS Cost Explorer API
                          ↓
                    Bedrock (Claude)
                          ↓
...
Before deploying, ensure you have:
 ...
 Cost Explorer enabled (wait 24 hours for initial data)
...
# Console → Cost Management → Cost Explorer → Enable Cost Explorer
Enable Bedrock Model Access
Go to AWS Console → Amazon Bedrock
Navigate to "Model access" in left sidebar
Click "Manage model access"
Select "Claude 3.5 Sonnet v2"
...
Cache Cost Explorer responses for 1-2 hours
...
Use API Gateway caching for frequently asked queries
🔧 Troubleshooting
...
Ensure correct model ID in variables
bash
# Check IAM policy
